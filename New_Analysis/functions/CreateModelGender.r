CreateModelGender <- function(dataComplete) {
  ...
  for (C in unique(dataComplete$country)) {
    ...
    # m_posrecip_tmp <- ulam(alist(posrecip_round ~ dordlogit(0, cutpoints),
    #                     cutpoints ~ dnorm(0, 1.5)),
    #               data = list(posrecip_round = dataComplete$posrecip_round), 
    #               chains = 4, cores = 3)
    #
    ...
    m_posrecip_pred_tmp <- ulam(alist(posrecip_round ~ dordlogit(phi, cutpoints),
                                      phi <- bG * gender,
                                      c(bG) ~ dnorm(0, 10),
    ...
    save(m_posrecip_pred_tmp, 
         file = paste0("tmp/models_gender/model_posrecip_gender_", C, ".RData"))
