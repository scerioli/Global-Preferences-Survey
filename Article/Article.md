...
In the present article, we replicated the results of the work by Falk&Hermle [@FH] that related of gender differences in economic preferences to economic development
and gender equality. The main finding of the original article can be summarized as follows. The authors used Gallup World Poll 2012 and linked Global Preference Survey datasets for the measurement of the gender diffrences in economic preferences across 76 counties. A joint index, summarizing the differences in all 6 preferences was used as a proxy for magnitude of gender differences. To associate the gender differences in economic preferences with the level of the economic development, the authors used Log GDP p/c as a proxy of economic development, while for gender equality several indexes were introduced, including a the onces previously used or discussed in literature, and a combined joint measure for gender equality was consructed. As the main result of the study the authors reported large and statistically significant association of gender differences with Log GDP p/c (r = 0.67, P < 0.0001) and the joint gender equality index (r = 0.56, P < 0.0001), as it was reflected as a main finding in the *Research article summary* and graphical abstract. The authors also conducted conditional analysis to isolate the impact of economic development and joint gender equality index. The association was found of smaller magnitude but still rather stond and statistically significant (r = 0.53, P < 0.0001) when gender differences were related to Log GDP p/c conditioned on joint gender equality index, and moderate and statistically significant (r = 0.32, P = 0.003) when relating with joint gender equality index and conditioning on Log GDP p/c. The authors concluded that the evidence indicates that higher levels of economic development and gender equality favor the manifestation of gender differences in preferences across countries, what highlighs *the critical role of avail-
ability of material and social resources, as well as gender-equal access to these resources, in facilitating the independent formation and expression of gender-specific preferences.*

Given the importance of this findings and thier impact, on one hand, as well as an impressive size and complexity of entier analysis, on the other, it is of high value to conduct the detailed reconstraction of the methodology, analysis, check the robustness of assamptions and inferences. As the first milestone, we conducted a pure replication with regared to the extraction of the gender differences in economic preferences from the Gallup World Poll 2012 and Global Preference Survey dataset, using the same methodology of the authors. The datasets are publicallaly availible only in preprocessed (qualitative and quantitative items of the quastionary were already mixed and standartized) and alas partially resticted form, that at some extend hampered the comparison of the replication with the original findings. Nevertheless, the extracted gender differences were close to the onces from the original article. In addition, we run the same analysis using robust regression instead of ordinary linear regression as the data revealed signs of non-normality and outliears: no signifinat changes in the destribution of gender differences in economic preferences were observed.  

As the next step, we investigated the indexes used to extimate economic developmet and gender equality, and thier relation to each other. First, we reveal a very stong association between Log GDP p/c and joint gender equality index (r = , P < ) and indexes approved by international organizations such as WEF, UNDP and GDI (r = , P < ; r = , P < ; r = , P < , respectively). This associaiton was previously highlighted by other authors [REFs] but did not recieve attention in the original manuscript. In the light of this strong relation and correlation between gender differences and Log GDP p/c reported by authors, the seconds main reported finding of strong and statistical significant association of gender differences with joint gender equality index became trivial as Log GDP p/c is a good predictor for gender equality indexes and vise versa. Only the conidtional regression with a control on Log GDP p/c may uncover the separate role of the gender equality on gender differences. 

Further thoughout the replication, we analysized the composition of the joint gender equality index composed by authors and investigated its single components. Several methodological flaws that are questiouning the validity of this procedure were identified. The joint gender equality indicator was composed from four other indicators, two widely used WEF and UNDP indicators (which in turn are build from 14 and 5 subindicators, respectively), the ratio of female-to-male partisipation, and also constructed a new type of the indicator "times since women suggrage".

Our analysis revealed that both WEF and UNDP already contein ratio of female to male forse partisipation as well as share subindicators related to the share seats in parlament and secondary education. The additional use of the female to male labor forse partification as a third component of the joint gender equaluty index remain unclear, as it prioretises this factor over all other factors included into indexes what leads to thier disbalance. In addition, the index constracted by authors by standartization of the years since women suffrage lack of justification and validation. Based on this findings and the absence of justification in the original article, we concluded that there is no evedence that joint equality measure performs not worse than existing and officially approved indexes, and free of biases due to inclusion of unvalidated indexes such as "time since women suffrage". Thus, in the further analysis we focused on the existing and widly accepted measures of the gender equality. 

The gender equolity indexes such as WEF, UNDP and GDI and themself have a certain limitations and were discussed by several authors [@SK; @Permanyer; @KPS; @AS; @Nicaragua; @10.1080/13545701.2010.530607], though the literature that debates the power and limitations of one of the most developmed indexes WEF is very sparse and limited, and the most of the relevant information can be found only in the regular reports of the authors of the index itself. Despite possible drawbacks, these indexes are the best estimates for the state of gender equality availible on the global scale in regular, standartized and systematic way. We proceeded with examinaiton of the gender differences in their relation to economic development and gender equality using these indexes and conditional analysis.

We found a large and statistically singificant associaiton of gender equality preferences with economic develpment conditioning on WEF and GDI (r = 0.6253, P < 0.0001 and r = 0.6612 , P < 0.0001, respectively), while for UNDP the association was somewhat smaller and with a larger p-value (r = 0.4190, P = 0.0194). Conditioning on economic development showed no correlation for UNDP and GDI indexes. While, for WEF the correlation was moderate with a mild statistical significance (r = 0.21, P = 0.024). We further analysed how single gender preferences are related to the economic develpment and gender differences. Along with the previous analysis, large and statistically significant associations demonstrated only in the situation when we related gender differences with Log GDP p/c conditioning on WEF and GDI. Intristinly, no preferences exept Altruism demonstrated any the relation with Log GDP p/c when conditioning on UNDP. Among 6 economic preferences and 3 gender equolity indexes, no statistically significant association between economic preferences and Log GDP conditioning on gender equality index  was found, exept for Altruism (r = 0.3561; P ) conditioning on WEF and Rist Taking (r = 0.2192; P ) when conditioning on UNDP, demonstrating little support of the presence of association of single gender differences in economic preferences and gender equality. 

Given these results of the reproduction analysis, we conlude that the evedence is not sufficient to support the claim about stong association between economic preferences and the level of gender equality. Several methodological approaches and assumptions used by authors in the original article should be reevaluated. The regression of the economic preferences on gender equality index can not be used as an actual measure of association due to high GDP-gender-equolity-indexes correlation, but rather coefficient from conditional analysis should be used. As suggest the conditional analysis, if a mild association still exists the magnitude of correlation coefficient is two-/three-fold smaller, with respect to the association of gender difference with the economic development conditioning on gender equality indexes.  
