...
In this piece of work, I will explore the following ideas:

1.  Interaction effects between Log GDP and Gender Equality Indexes
2.  Use of other economic development indicators like the HDI and the GNI
3.  Split the HDI sub-indexes to understand which contributes more
...
This first part of the study is done only for the purpose of understanding the contribution to the original article. It has been criticized in our previous paper how using the GEI is not meaningful.
...
When creating the models, we can also have a look at their summary and then perform an Analysis Of the Variance (ANOVA) to understand if there are relevant terms to be added to the model:
...
Another interesting observation is that the models with interactions are more coherent to each other: the GDI gives now some statistically significant p-value as for the other models, and all have a similar magnitude:
...
gni_lri3 <- lm(avgGenderDiffStd ~ ValueUNStd * logAvgGNIStd, 
           data = summaryIndex)
gni_lri3_out <- lm(avgGenderDiffStd ~ ValueUNStd * logAvgGNIStd, 
           data = summaryIndex[!(country %in% c("Saudi Arabia", "United Arab Emirates"))])
gni_rlri3 <- rlm(avgGenderDiffStd ~ ValueUNStd * logAvgGNIStd, 
           data = summaryIndex)
summary(gni_lri3)
summary(gni_lri3_out)
summary(gni_rlri3)
...
```{r}
summary(lm(logAvgGDPpcStd ~ logAvgGNIStd, data = summaryIndex))
PlotSummary(data = summaryIndex,
            var2 = "logAvgGDPpcStd", var1 = "logAvgGNIStd",
            display = TRUE)
```
...
All seems caused by the effects of two outliers Saudi Arabia and UAE! So one can solve it in 2 ways, either removing the outliers, or using a robust linear regression.

Removing the outliers is not really a good way to deal with real data (they are like this and there is no error in them), so I will go for the robust linear regression. It is needed to be done for all the models. Also, add a function in order to have the p-value displayed.

```{r}
ggplot(summaryIndex) +
  geom_histogram(aes(x = logAvgGDPpcStd))

ggplot(summaryIndex) +
  geom_histogram(aes(x = HDIStd))

ggplot(summaryIndex) +
  geom_histogram(aes(x = logAvgGNIStd))
```

Do they follow the assumption of being normally distributed?

```{r}
qqnorm(summaryIndex$logAvgGDPpcStd)
qqline(summaryIndex$logAvgGDPpcStd)

qqnorm(summaryIndex$HDIStd)
qqline(summaryIndex$HDIStd)

qqnorm(summaryIndex$logAvgGNIStd)
qqline(summaryIndex$logAvgGNIStd)
```

```{r}
shapiro.test(summaryIndex$logAvgGDPpcStd)
shapiro.test(summaryIndex$HDIStd)
shapiro.test(summaryIndex$logAvgGNIStd)

shapiro.test(summaryIndex$avgGenderDiffStd)

shapiro.test(summaryIndex$GenderIndexStd)
shapiro.test(summaryIndex$ScoreWEFStd)
shapiro.test(summaryIndex$ValueUNStd)
shapiro.test(summaryIndex$GDIStd)
```

```{r}
ggplot(summaryIndex) +
  geom_histogram(aes(x = GenderIndexStd))

ggplot(summaryIndex) +
  geom_histogram(aes(x = ScoreWEFStd))

ggplot(summaryIndex) +
  geom_histogram(aes(x = ValueUNStd))

ggplot(summaryIndex) +
  geom_histogram(aes(x = GDIStd))
```

```{r}
qqnorm(summaryIndex$GenderIndexStd)
qqline(summaryIndex$GenderIndexStd)

qqnorm(summaryIndex$ScoreWEFStd)
qqline(summaryIndex$ScoreWEFStd)

qqnorm(summaryIndex$ValueUNStd)
qqline(summaryIndex$ValueUNStd)

qqnorm(summaryIndex$GDIStd)
qqline(summaryIndex$GDIStd)
```

Based on the distributions, I can say that taking the WEF GGGI seems to be the optimal choice when computing a linear model. I use also the log GDP p/c and the GNI p/c and check the results (both of them were not rejected to be different to H0):

```{r}

model <- lm(avgGenderDiffStd ~ ScoreWEFStd*logAvgGDPpcStd, data = summaryIndex)
model2 <- lm(avgGenderDiffStd ~ ScoreWEFStd*logAvgGNIStd, data = summaryIndex)
...
The models have similar coefficients and statistical significance overall.
