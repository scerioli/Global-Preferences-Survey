AddResidualsSinglePreference_new <- function(dt, robust = FALSE) {
  ...
  for (pref in unique(dt$preference)) {
    ...
    # using the Gender Equality Index Rescaled
    ...
    list_dt <- list(dt_tmpGDP, dt_tmpGEI, 
                    dt_tmpWEF, dt_tmpUN, dt_tmpGDI,
                    dt_logGDP_GEI, dt_GEI_logGDP, 
                    dt_logGDP_WEF, dt_WEF_logGDP,
                    dt_logGDP_UN, dt_UN_logGDP, 
                    dt_logGDP_GDI, dt_GDI_logGDP)
