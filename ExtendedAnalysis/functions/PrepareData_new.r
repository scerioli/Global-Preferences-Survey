PrepareData_new <- function(data_all) {
  ...
  GDI_index     <- as.data.table(data_all$GDI_index)
  ...
  ## --------------------------- 1.6 GDI Index -------------------------------- ##
  setnames(GDI_index, old = "Country", new = "country")
  
  # Clean the country names
  GDI_index[country %like% "Bosnia and Herzegovina", country := "Bosnia Herzegovina"]
  GDI_index[country %like% "Iran", country := "Iran"]
  GDI_index[country %like% "Russian Federation", country := "Russia"]
  GDI_index[country %like% "Korea", country := "South Korea"]
  GDI_index[country %like% "Venezuela", country := "Venezuela"]
  GDI_index[country %like% "Bolivia", country := "Bolivia"]
  GDI_index[country %like% "Czechia", country := "Czech Republic"]
  GDI_index[country %like% "Moldova", country := "Moldova"]
  GDI_index[country %like% "Viet Nam", country := "Vietnam"]
  GDI_index[country %like% "Tanzania", country := "Tanzania"]
  # ---------------------------------------------------------------------------- #
